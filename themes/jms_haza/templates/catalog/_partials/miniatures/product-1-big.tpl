...
<div class="product-miniature js-product-miniature productbox-1" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">
	<div class="product-preview">
		{block name='product_thumbnail'}
            ...
            {if $jmsSetting.carousel_lazyload}data-src="{$product.cover.bySize.large_default.url}"{else}
            ...
            {/if}
              ...
              {if $jmsSetting.carousel_lazyload}data-src="{$product.images.1.bySize.large_default.url}"{else}
	...
	<div class="product-info">
        {if $jmsSetting.productbox_category}
          ...
          {else}
            <h3 class="product-title" itemprop="name"><a class="product-link" href="{$product.canonical_url}">{$product.name|truncate:30:'...'}</a></h2>
          {/if}
        {/block}
        ...
        <div class="product_button">
            ...
            {else}
            <button class="btn-default product-btn disabled" disabled title="{l s='Sold Out' d='Shop.Theme.Actions'}" data-id-product="{$product.id}" data-minimal-quantity="{$product.minimal_quantity}">
