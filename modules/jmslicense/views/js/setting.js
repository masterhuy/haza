...
jQuery(function ($) {
	/*$(document).on('click','#jms-setting-tabs > .tab > a',function(event) {
			var liparent = $(this).parent();
			var settingtab = $('#jms-setting-tabs .tab');
			settingtab.removeClass('active');
			liparent.addClass('active');
			if(liparent.hasClass('haschild')) {
					liparent.find('.tab').eq(0).addClass('active');
					...
					liparent.find('ul').slideDown({
							complete: function() {
									...
									$(liparent.find('.tab a').eq(0).attr('href')).addClass('active');
							}
					}, 600);
					//liparent.find('ul').slideDown(600);
			} else {
					$(this).children('i').html('add');

			}
	});*/
	$(document).on('click','#jms-setting-tabs > .tab > a',function(e) {
			e.preventDefault();
			e.stopPropagation();
			let $submenu = $(this).parent();
			$('body').toggleClass('setting-sidebar-closed');
			let onlyClose = $(e.currentTarget).parent().hasClass('ul-open');
			$('#jms-setting-tabs li.haschild a > i.material-icons.ps-togglable-row')
					.text('keyboard_arrow_down');
			if ($('body').is('.setting-sidebar-closed:not(.mobile)')) {
					$('#jms-setting-tabs > .tab.ul-open').removeClass('ul-open open');
					$('#jms-setting-tabs li.haschild.ul-open ul').removeAttr('style');
			} else {
					$('#jms-setting-tabs li.haschild.ul-open ul').slideUp({
							complete: function() {
	...
	$('.js-dialog-upload').fancybox({
			...
			beforeClose: function beforeClose() {
					...
					var val = $input.val();
					$input.val(val.replace($('#jms-root-url').val(), ""));
					$input.change();
			}
	});
...
$(document).ready(function() {
	/*$("#importSetting").click(function(d) {
			d.preventDefault();
			d.stopPropagation();
	...
	$('.g-font-family').on('change', function() {
			...
			var ajaxurl = $('#jms-base-url').val() + 'modules/jmsthemesetting/ajax_run.php?action=load_font_weight&fontfamily=' + fontfamily;
			$.ajax({
	      url: ajaxurl,
	      method: 'GET',
				dataType: 'json',
