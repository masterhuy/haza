...
jQuery(function ($) {
	...
	$(document).on('change',"#select-page",function(event) {
		document.location = $('#backend_url').val() + '&config_id_page=' + $(this).val();
	})
	$(document).on('click','#page-header-desc-configuration-save',function(event) {
		...
		/*$.ajax({
                type: 'POST',
                headers: {"cache-control": "no-cache"},
                url: $('#ajax_url').val() + '?action=SaveLayout' + '&id_page=' + $('#rowlist').data('id'),
                async: true,
                cache: false,
                data: 'action=SaveLayout&params=' + params + '&id_page=' + $('#rowlist').data('id'),
				dataType: 'json',
                success: function() {
