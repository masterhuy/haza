  ...
  <div class="tabs-navigation">
    <ul class="nav nav-tabs" role="tablist">
  		{foreach from = $categories key = k item = category}
  			<li class="nav-item"><a data-toggle="tab" href="#category-{$category.id_category nofilter}" class="nav-link{if $k == 0} active{/if}">{$category.name|escape:'htmlall':'UTF-8'}</a></li>
  		{/foreach}
  	</ul>
  </div>
</div>
