		...
		{foreach from=$posts item=post}
			...
			<div class="blog-item">
				...
				<ul class="post-meta">
					...
					{/if}
					{if $show_time == '1'}
						<li class="post-created"><i class="fa fa-calendar-o"></i> {$post.created|escape:'html':'UTF-8'|date_format:'%b %e, %Y'}</li>
					{/if}
					{if $show_nviews == '1'}
						<li class="post-views"><i class="fa fa-eye"></i> {$post.views nofilter}</li>
					{/if}
					{if $show_ncomments == '1'}
						<li class="post-comments"><i class="fa fa-comment-o"></i> {$post.comment_count nofilter}</li>
					{/if}
				</ul>
				{if $show_readmore == '1'}
					<a class="btn post-readmore">{l s='read more' d='Modules.JmsPagebuilder'}</a>
				{/if}
			</div>
		{/foreach}
	</div>
