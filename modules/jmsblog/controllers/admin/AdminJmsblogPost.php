...
class AdminJmsblogPostController extends ModuleAdminController
{
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitPost')) {
			...
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminJmsblogPost', true).'&conf=3&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        } elseif (Tools::isSubmit('delete_id_post')) {
    ...
    public function getPostCount($category_id = 0, $state = -1)
    {
        ...
        $sql = '
            ...
            FROM '._DB_PREFIX_.'jmsblog_posts hss
            INNER JOIN '._DB_PREFIX_.'jmsblog_posts_lang hssl ON (hss.`post_id` = hssl.`post_id`)
            INNER JOIN '._DB_PREFIX_.'jmsblog_categories_lang hssll ON (hss.`category_id` = hssll.`category_id` AND hss.`category_id` IS NOT NULL)
            WHERE hssl.`id_lang` = '.(int)$id_lang.' AND hssll.`id_lang` = '.(int)$id_lang.
            $filter.
            ($category_id ? ' AND hss.`category_id` = '.$category_id : ' ').'
    ...
    public function renderAddPost()
    {
        ...
        $this->getCategorySelect(0, 0);
        $categories = $this->catselect;
        //array_unshift($categories, array ( 'category_id' => $category_id,'title' => 'No-Category' ));
