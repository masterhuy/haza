...
class JmsBlogHelper
{
    ...
    public static function getCategory($category_id = 0)
    {
        ...
        $_query = 'SELECT hss.`category_id` as category_id, hssl.`title`, hssl.`alias` FROM '._DB_PREFIX_.'jmsblog_categories hss LEFT JOIN '._DB_PREFIX_.'jmsblog_categories_lang hssl ON (hss.category_id = hssl.category_id) WHERE hss.active = 1 AND hssl.id_lang = '.(int)$id_lang.' AND hss.`category_id` = '.$category_id.' ORDER BY hss.ordering';
    ...
    public static function getCategoryByAlias($category_slug = '')
    {
        $context = Context::getContext();
        $id_lang = $context->language->id;
        $_query = 'SELECT hss.`category_id` as category_id, hssl.`title` FROM '._DB_PREFIX_.'jmsblog_categories hss LEFT JOIN '._DB_PREFIX_.'jmsblog_categories_lang hssl ON (hss.category_id = hssl.category_id) WHERE hss.active = 1 AND hssl.id_lang = '.(int)$id_lang.' AND hssl.`alias` = "'.$category_slug.'" ORDER BY hss.ordering';
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($_query);
    }
    ...
    {
        return array(
            'JMSBLOG_PAGE_SIDEBAR' => Tools::getValue('JMSBLOG_PAGE_SIDEBAR', Configuration::get('JMSBLOG_PAGE_SIDEBAR')),
